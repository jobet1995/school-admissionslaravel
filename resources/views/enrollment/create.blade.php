...
@section('content')
<div class="container">
    <form method="post" action="/admin/enrollments?ref={{urlencode($ref)}}">
        @csrf
        <div class="row">
            <div class="col m6 l4">
                <label for="enrollment_student">Student</label>
                <input id="enrollment_student" name="student" value="{{old('student')}}" type="number" />
                @error('student')<span class="red-text">{{$message}}</span>@enderror
            </div>
            <div class="col m6 l4">
                <label for="enrollment_course">Course</label>
                <input id="enrollment_course" name="course" value="{{old('course')}}" type="number" />
                @error('course')<span class="red-text">{{$message}}</span>@enderror
            </div>
            <div class="col m6 l4">
                <label for="enrollment_enrollment_date">Enrollment Date</label>
                <input id="enrollment_enrollment_date" name="enrollment_date" value="{{old('enrollment_date')}}" data-type="date" autocomplete="off" />
                @error('enrollment_date')<span class="red-text">{{$message}}</span>@enderror
            </div>
            <div class="col m6 l4">
                <label for="enrollment_grade">Grade</label>
                <input id="enrollment_grade" name="grade" value="{{old('grade')}}" maxlength="50" />
