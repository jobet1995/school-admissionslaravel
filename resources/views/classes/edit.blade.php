...
@section('content')
<div class="container">
    <form method="post" action="/admin/classes/{{$class->class_id}}?ref={{urlencode($ref)}}">
        @method("PATCH")
        @csrf
        <div class="row">
            <div class="col m6 l4">
                <label for="class_class_id">Class Id</label>
                <input readonly id="class_class_id" name="class_id" value="{{old('class_id', $class->class_id)}}" type="number" required />
                @error('class_id')<span class="red-text">{{$message}}</span>@enderror
            </div>
            <div class="col m6 l4">
                <label for="class_student">Student</label>
                <input id="class_student" name="student" value="{{old('student', $class->student)}}" type="number" />
                @error('student')<span class="red-text">{{$message}}</span>@enderror
            </div>
            <div class="col m6 l4">
                <label for="class_course">Course</label>
                <input id="class_course" name="course" value="{{old('course', $class->course)}}" type="number" />
